...
if _G.ConfigUse then
    ...
    if config then
        ...
        if config["Devil Fruit"] then
            DF = true
        end
    else
        warn("Config không tồn tại hoặc chưa được khởi tạo")
    end
end
-------Cusc----------
local osfunc = {}
    ...
    while wait() do
        if _G.AutoFarmBounty then
            for i,v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
                if not v:FindFirstChild("Humanoid") or not v:FindFirstChild("HumanoidRootPart") or v.Humanoid.Health == 0 then
                    NextplySelect = true
                    wait(.2)
                end
            end
            ...
            if _G.AutoFarmBounty then
                for i, v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
                    if v.Name ~= game.Players.LocalPlayer.Name then
                        ...
                        if humanoid and humanoidRootPart and humanoid.Health > 0 and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - humanoidRootPart.Position).Magnitude <= 10000 then
                            plyselecthunthelpold = humanoid.Health
                            repeat task.wait()
                                NameTarget = v.Name
                                Converted["_Target Name"].Text = "Target Name : "..v.Name..""
                                if tostring(game.Players.LocalPlayer.Team) == "Pirates" then
                                    topos(humanoidRootPart.CFrame * CFrame.new(0, 4, 2))
                                ...
                                spawn(function()
                                    if (humanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 15 then
                                        spawn(function()
                                            autoskill()
                                            Click()
                                            Aimbot = true
                                ...
                                TargetSelectHunt = humanoid
                            until _G.AutoFarmBounty == false or humanoid.Health == 0 or not humanoidRootPart or not humanoid or not v.Parent or NextplySelect == true
                            NextplySelect = false
